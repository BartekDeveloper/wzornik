import { ONE, ZERO, add, binom, cmp, div, factorial, gcd, isIntegerR, isZero, mul, neg, of, pow, sub } from '../exact/rational'
import type { Rational } from '../exact/rational'
import { approx, approxOnly, cbrtRational, exactOf, logExact, solveQuadratic, sqrtRational } from '../exact/exact'
...
const pierwiastek: FormulaDef = {
  ...
  solve(unknown, known, places): FormulaSolution {
    ...
    if (n === 2) {
      if (cmp(m, ZERO) < 0) throw new Error('pierwiastek kwadratowy z ujemnej nie istnieje (na rzeczywistych)')
    ...
    try {
      ...
      if (e instanceof Error && e.message !== 'no-exact' && !e.message.includes('dokładnie')) throw e
      if (cmp(m, ZERO) < 0 && n % 2 === 0) throw new Error('parzysty pierwiastek z ujemnej nie istnieje')
...
const ukladRownan: FormulaDef = {
  ...
  solve(_unknown, known, places): FormulaSolution {
    ...
    return {
      ...
      labels: ['x', 'y'],
      steps: [
        { title: '1. Wyznaczniki (Cramer)', body: 'W = a_1b_2 - a_2b_1, \\; x = \\frac{W_x}{W}, \\; y = \\frac{W_y}{W}' },
        {
...
const nierownoscKwadratowa: FormulaDef = {
  ...
  solve(_unknown, known, places): FormulaSolution {
    ...
    if (q.kind === 'one' || q.kind === 'linear') {
      ...
      return up
        ? interval(`x \\in \\mathbb{R} \\setminus \\{${x0}\\}`, 'pierwiastek podwójny nie spełnia >')
...
const hornerPierwiastki: FormulaDef = {
  ...
  solve(_unknown, known, places): FormulaSolution {
    ...
    const head = [
      { title: '1. Kandydaci (tw. o pierwiastkach wymiernych)', body: 'x = \\pm\\frac{p}{q}, \\; p \\mid d, \\; q \\mid a' },
      {
        title: '2. Sprawdzenie',
    ...
    if (roots.length === 0) {
      return {
        values: [],
        steps: [...head, { title: '3. Wynik', body: '\\varnothing', note: 'brak pierwiastków wymiernych' }],
      }
    }
    ...
    return {
      ...
      steps: [
        ...
        {
          title: '3. Wynik',
          body: roots.map((r, i) => `x_{${i + 1}} = ${formatLatex({ rat: r, irr: null })}`).join(', \\; '),
...
const postacKanoniczna: FormulaDef = {
  ...
  solve(_unknown, known, places): FormulaSolution {
    ...
    const v = (e: Exact): string => {      const lx = formatLatex(e)
    ...
    return {
      ...
      steps: [
        ...
        { title: '3. Wynik', body: `W = (${v(p)}, ${v(q)}), \\; f(x) = ${L(a)}(x - ${v(p)})^2 + ${v(q)}` },
      ],
    }
  },
}
  ...
  solve(_unknown, known, places): FormulaSolution {
    ...
    return {
      ...
      steps: [
        ...
        { title: '2. Podstawienie danych', body: `C^{${k}}_{${n}} = \\frac{${n}!}{${k}!(${n}-${k})!}` },
...
const postacieKwadratowej: FormulaDef = {
  ...
  solve(_unknown, known, places): FormulaSolution {
    ...
    const steps: FormulaSolution['steps'] = [
      ...
      { title: '2. Postać kanoniczna', body: `f(x) = ${kanoniczna}, \\; W = (${show(p)}, ${show(q)})` },
    ]
...
const rozkladWielomianu: FormulaDef = {
  ...
  solve(_unknown, known, places): FormulaSolution {
    ...
    const lin = (r: Rational): string =>
      cmp(r, ZERO) === 0 ? 'x' : cmp(r, ZERO) > 0 ? `(x-${show({ rat: r, irr: null })})` : `(x+${show({ rat: neg(r), irr: null })})`
    if (isZero(a)) {
      ...
      if (q.kind === 'none') {
        return {
          ...
          steps: [
            ...
            { title: '3. Wynik', body: `${L(b)}x^2+${L(c)}x+${L(d)}`, note: 'nierozkładalny nad ℝ (Δ < 0)' },
          ],
        }
      }
      const roots = q.roots
    ...
    if (D === 0n) {
      return {
        ...
        steps: [
          ...
          { title: '3. Wynik', body: `W(x) = x(${L(a)}x^2+${L(b)}x+${L(c)})`, note: 'dalej rozłóż trójmian (delta)' },
        ],
      }
    }
    let coeffs: Rational[] = [
