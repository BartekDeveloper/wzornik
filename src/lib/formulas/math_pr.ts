import { ONE, ZERO, add, binom, cmp, div, factorial, isIntegerR, isZero, mul, neg, of, pow, sub } from '../exact/rational'
    ...
    if (eq1) {
      ...
      return {
        ...
        steps: [
          ...
          { title: '2. Podstawienie danych', body: `q = ${L(q)}` },
          { title: '3. Wynik', body: '-1, 1, -1, \\ldots', note: 'granica nie istnieje — ciąg oscyluje' },
        ],
      }
    }
    return {
      ...
      steps: [
        ...
        { title: '2. Podstawienie danych', body: `|${L(q)}| > 1` },
        {
          ...
          note: cmp(q, ZERO) > 0 ? 'rozbieżny do +∞' : 'rozbieżny co do wartości (znak na przemian)',
        },
      ],
    }
  },
...
const pochodnaWielomianu: FormulaDef = {
  ...
  solve(_unknown, known, places): FormulaSolution {
    ...
    const body = `${term(A, 'x^2', true)}${term(B, 'x', A.p === 0n)}${term(c, '', A.p === 0n && B.p === 0n)}`.trim() || '0'
    return {
...
const podwojonyKat: FormulaDef = {
  ...
  solve(_unknown, known, places): FormulaSolution {
    ...
    return {
      ...
      steps: [
        { title: '1. Wzory', body: '\\sin 2x = 2\\sin x\\cos x, \\; \\cos 2x = \\cos^2 x - \\sin^2 x' },
        { title: '2. Podstawienie danych', body: `x = ${L(asRational(raw, 'x'))}^\\circ` },
        { title: '3. Wynik', body: `\\sin ${2 * deg}^\\circ = ${show(s)}, \\; \\cos ${2 * deg}^\\circ = ${show(c)}` },
      ],
    }
  },
}
