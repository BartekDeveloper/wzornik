...
const ruchJednostajniePrzyspieszony: FormulaDef = {
  ...
  solve(unknown, known, places) {
    ...
    if (q.kind !== 'two' && q.kind !== 'one') throw new Error('brak rozwiązania dla czasu (Δ < 0) — sprawdź dane')
...
const energiaKinetyczna: FormulaDef = {
  ...
  solve(unknown, known, places) {
    if (unknown === 'Ek') {
      ...
      return {
        values: [value],
        steps: stdSteps('E_k = \\frac{mv^2}{2}', `E_k = \\frac{${L(m)} \\cdot ${L(v)}^2}{2}`, value, places),
      }
    }
    if (unknown === 'm') {
      ...
      return {
        values: [value],
        steps: stdSteps('m = \\frac{2E_k}{v^2}', `m = \\frac{2 \\cdot ${L(Ek)}}{${L(v)}^2}`, value, places),
      }
    }
    const Ek = asRational(known['Ek'], 'Eₖ')
    const m = asRational(known['m'], 'm')
    ...
    return {
      values: [value],
      steps: stdSteps('v = \\sqrt{\\frac{2E_k}{m}}', `v = \\sqrt{\\frac{2 \\cdot ${L(Ek)}}{${L(m)}}}`, value, places),
    }
  },
}
...
const energiaPotencjalna: FormulaDef = {
  ...
  solve(unknown, known, places) {
    ...
    if (unknown === 'm') {
      ...
      return {
        values: [value],
        steps: stdSteps('m = \\frac{E_p}{gh}', `m = \\frac{${L(Ep)}}{10 \\cdot ${L(h)}}`, value, places),
      }
    }
    const Ep = asRational(known['Ep'], 'Eₚ')
    const m = asRational(known['m'], 'm')
    ...
    return {
      values: [value],
      steps: stdSteps('h = \\frac{E_p}{gm}', `h = \\frac{${L(Ep)}}{10 \\cdot ${L(m)}}`, value, places),
    }
  },
}
